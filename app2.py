...
START (啟動關鍵字): 例如「啟動」、「你好」等等
STOP (結束關鍵字): 例如「關閉」、「再見」等等
INTERRUPT (打斷關鍵字): 例如「等一下」等等
    ...
    async def handle_transcript_event(self, transcript_event: TranscriptEvent):
        for result in transcript_event.transcript.results:
            ...
            if text:
                ...
                classfify=LiveTranscriber()
                await result=classfify.flush_buffer(text)
                if result:
                    await cancellable_socket_handle_text(text)
                else:
                    print("⚠️ 目前不在啟動狀態，忽略文字：", text)
        


class LiveTranscriber:
    def __init__(self, region="us-west-2", callback=None, silence_timeout=0.5):
    ...
    async def classify_intent(self, text: str) -> str:
        ...
        def _invoke():
            ...
            if isinstance(content, list) and content:
                result = content[0].get("text", "").strip().upper()
            else:
    ...
    # ---------- 停頓後送出 ----------
    async def flush_buffer(self,text: str):
